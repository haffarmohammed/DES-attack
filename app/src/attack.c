...
DATA initialize_data(){
	DATA d;
	uint64_t chiffre_juste = 0xC73882CF364F444F ; //mon chiffre juste
	...
	d.message_clair=0x201031D5E6E6FDC2;
...
int attack_sbox(DATA* data, uint8_t* uint48_t_part, int num_sbox){
	...
	printf("\033[1m\033[32m\nSearching in S-BOX n °%d : \n\n\033[0m", num_sbox);
	for(s=0;s<32;s++){
		h=0;
		if (candidate_key[s][64]==1){
			printf("Encrypted false n °%d : ", s+1);
			for(w=0;w<64;w++){
			...
			printf("--> %d possible sub-key portions\n", h);
		}
	...
	*uint48_t_part=find_intersection(selection_key);
	printf("\033[1m\033[34mKey portion found  : %x\n\n\033[0m", *uint48_t_part);
...
int find_K16(DATA* data){
	...
	printf("\033[1m\033[33m\n-------------------------------- K16 key found  : %LX ----------------------------\033[0m", data->k16.bytes);
	return 0;
}
...
int find_K(DATA* data){
	...
	for (i=0;i<0xFF; i++){
		...
		if ((data->message_clair)==data->chiffre_juste.output){
			...
			printf(" -------------------------\n\033[0m");
			printf("\033[1m\033[31m------------------------------------ HAFFAR Mohammed 22010473 ----------------------------- \n\n\033[0m");
			return 0;
		}
	}
	return 1;
